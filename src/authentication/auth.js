...
async function generateJWTToken (request, env) {
    ...
    const savedPass = await env.nsa.get('pwd');
    if (password !== savedPass) return new Response('Method Not Allowed', { status: 405 });
    let secretKey = await env.nsa.get('secretKey');
    if (!secretKey) {
        secretKey = generateSecretKey();
        await env.nsa.put('secretKey', secretKey);
    }
...
export async function Authenticate (request, env) {
    try {
        const secretKey = await env.nsa.get('secretKey');
        const secret = new TextEncoder().encode(secretKey);
...
export async function resetPassword(request, env) {
    ...
    const oldPwd = await env.nsa.get('pwd');
...
export async function login(request, env) {
    await initializeParams(request, env);
    if (typeof env.nsa !== 'object') return await renderErrorPage(request, env, 'KV Dataset is not properly set!', null, true);
