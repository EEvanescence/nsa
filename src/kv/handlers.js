...
export async function getDataset(request, env) {
    ...
    if (typeof env.nsa !== 'object') {
        return {kvNotFound: true, proxySettings: null, warpConfigs: null}
    }

    try {
        proxySettings = await env.nsa.get("proxySettings", {type: 'json'});
        warpConfigs = await env.nsa.get('warpConfigs', {type: 'json'});
    } catch (error) {
        console.log(error);
...
export async function updateDataset (request, env) {
    ...
    if (!isReset) {
        try {
            currentSettings = await env.nsa.get("proxySettings", {type: 'json'});
        } catch (error) {
            console.log(error);
            throw new Error(`An error occurred while getting current KV settings - ${error}`);
        }
    } else {
        await env.nsa.delete('warpConfigs');
        newSettings = null;
    }
    ...
    try {    
        await env.nsa.put("proxySettings", JSON.stringify(proxySettings));          
    } catch (error) {
        console.log(error);
