...
async function buildXrayBestPingConfig(proxySettings, totalAddresses, chainProxy, outbounds, isFragment) {
    const remark = isFragment ? '💦 nsa F - Best Ping 💥' : '💦 nsa - Best Ping 💥';
...
async function buildXrayBestFragmentConfig(proxySettings, hostName, chainProxy, outbounds) {
    ...
    const config = buildXrayConfig(proxySettings, '💦 nsa F - Best Fragment 😎', true, true, chainProxy, false, false);
...
async function buildXrayWorkerLessConfig(proxySettings) {
    const config = buildXrayConfig(proxySettings, '💦 nsa F - WorkerLess ⭐', true, false, false, false, false);
...
export async function getXrayCustomConfigs(request, env, isFragment) {
    ...
    if (outProxy) {
        ...
        try {
            ...
            await env.nsa.put("proxySettings", JSON.stringify({
                ...proxySettings, 
                outProxy: '',
                outProxyParams: {}
            }));
