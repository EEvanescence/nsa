...
async function buildXrayBestFragmentConfig(proxySettings, hostName, chainProxy, outbounds) {
    ...
    const config = buildXrayConfig(proxySettings, '☆ Fragment Best Ping', true, true, chainProxy, false, false);
...
async function buildXrayWorkerLessConfig(proxySettings) {
    const config = buildXrayConfig(proxySettings, '🐓 WorkerLess Config', true, false, false, false, false);
...
export async function getXrayWarpConfigs (request, env, client) {
    ...
    for (const [index, endpoint] of warpEndpoints.split(',').entries()) {
        ...
        const warpConfig = buildXrayConfig(proxySettings, `⚫️ ${index + 1} - Warp${proIndicator}`, false, false, false, false, true);
